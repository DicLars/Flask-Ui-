...
def index():
    ...
    Z = df2.groupby(['Content Rating','Type']).agg({'Installs':'sum'}).sort_values(by='Installs',ascending=False).head(4)
    Z2= df2.groupby(['Content Rating','Type']).agg({'Installs':'sum'}).sort_values(by='Installs',ascending=False).tail(6)
    Z1=Z.reset_index()
    ...
    plt.subplot(1,2,1).bar(x=Z1True.index,height=Z1True['Installs'],color=my_colors)
    plt.xlabel('Rating and Type')
    plt.ylabel('# of Installs')
    plt.title('Top 4 Installs')
    plt.xticks(rotation=90)
    plt.gca().yaxis.set_major_formatter(ticker.StrMethodFormatter('{x:,.0f}')) # comma separator untuk angka
    plt.tight_layout(w_pad=10)
    plt.subplot(1,2,2).bar(x=Z2True.index,height=Z2True['Installs'],color=my_colors)
    plt.title('Low 6 Installs')
    plt.xlabel('Rating and Type')
    plt.ylabel('# of Installs')
    plt.xticks(rotation=90)
    plt.gca().yaxis.set_major_formatter(ticker.StrMethodFormatter('{x:,.0f}')) # comma separator untuk angka
